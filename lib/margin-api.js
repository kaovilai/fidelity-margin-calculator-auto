...
const MarginAPI = (() => {
  ...
  const RETRY_JITTER_BASE = 0.5; // multiplier range: [0.5, 1.5) to spread retries
...
fragment ImageDetails on Image {
  ...
  function getXsrfToken() {
    const match = document.cookie.split('; ').find(c => c.startsWith('MARGIN-CALCULATOR-XSRF-TOKEN='));
    return match ? match.slice('MARGIN-CALCULATOR-XSRF-TOKEN='.length) : null;
  }
  ...
  async function fetchWithRetry(accountNum, orders, onRetry, priceList) {
    for (let attempt = 0; attempt <= RETRY_DELAYS.length; attempt++) {
      try {
        ...
        const delay = RETRY_DELAYS[attempt] * (RETRY_JITTER_BASE + Math.random());
