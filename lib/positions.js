...
`;
  ...
  const RETRY_JITTER_BASE = 0.5; // multiplier range: [0.5, 1.5)
  ...
  async function fetchPriceListWithRetry(accountNum) {
    for (let attempt = 0; attempt <= RETRY_DELAYS.length; attempt++) {
      try {
        return await doFetchPriceList(accountNum);
      } catch (err) {
        if (!RETRYABLE_TYPES.has(err.type)) throw err;
        if (attempt >= RETRY_DELAYS.length) throw err;
        const delay = RETRY_DELAYS[attempt] * (RETRY_JITTER_BASE + Math.random());
        debugLog(`Positions retry ${attempt + 1}/${RETRY_DELAYS.length} in ${Math.round(delay)}ms`);
