...
`;
  ...
  const RETRYABLE_TYPES = new Set([FMC_CONSTANTS.ERROR_TYPES.NETWORK_ERROR]);
  ...
  async function doFetchPriceList(accountNum) {
    ...
    if (!resp.ok) {
      ...
      if (resp.status === 429 || resp.status >= 500) {
        ...
        throw new PositionsError(
          ...
          FMC_CONSTANTS.ERROR_TYPES.NETWORK_ERROR
        );
      }
    ...
    const priceList = positions
      .filter(p => p.cusip && typeof p.quantity === 'number' && p.marketValDetail && typeof p.marketValDetail.marketVal === 'number')
